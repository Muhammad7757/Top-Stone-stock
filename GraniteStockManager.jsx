...
export default function GraniteStockManager() {
  ...
  useEffect(()=>{ try{ const s=localStorage.getItem(STORAGE_KEYS.SLABS); const b=localStorage.getItem(STORAGE_KEYS.BLOCKS); if(s)setSlabs(JSON.parse(s)); if(b)setBlockNumbers(JSON.parse(b)); }catch(e){console.error(e);} },[]);
  useEffect(()=>{ localStorage.setItem(STORAGE_KEYS.SLABS,JSON.stringify(slabs)); },[slabs]);
  useEffect(()=>{ localStorage.setItem(STORAGE_KEYS.BLOCKS,JSON.stringify(blockNumbers)); },[blockNumbers]);
  function clearForm(){ setBlockNumber(''); setLength(''); setSelectedColor(colors[0]); setSelectedWidth(widths[0]); }
  function handleAddSlab(){
    if(!blockNumber.trim()){alert('Block number required'); return;}
    if(!length||isNaN(Number(length))){alert('Length required (numeric, inches)'); return;}
  ...
  function handleDeleteSlab(id){ if(!confirm('Are you sure?')) return; setSlabs(prev=>prev.filter(s=>s.id!==id)); }
  function exportCSV(){
  ...
  function importJSON(file){ const reader=new FileReader(); reader.onload=e=>{ try{ const data=JSON.parse(e.target.result); if(Array.isArray(data)){ setSlabs(prev=>[...data,...prev]); const newBlocks=data.map(d=>d.blockNumber).filter(Boolean); setBlockNumbers(prev=>Array.from(new Set([...newBlocks,...prev]))); alert('Import complete'); }else{alert('JSON must be array');} }catch(err){alert('Invalid JSON');} }; reader.readAsText(file); }
